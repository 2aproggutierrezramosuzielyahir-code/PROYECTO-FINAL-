...
productos = []
ventas = []
promociones = []  # <-- NUEVO: lista de promociones
...
def ventana_inicio():
    ...
    def frame_login():
        ...
        def validar():
            nonlocal perfil_index, productos_index, caja_index, atencion_index, promo_index
            global logged_in
            ...
            for u in usuarios:
                if usuario_entry.get() == u["usuario"] and password_entry.get() == u["password"]:
                    ...
                    menu.entryconfig(promo_index, state="normal")
    ...
    # ============================================================
    # GESTIÓN DE DESCUENTOS Y PROMOCIONES (NUEVO)
    # ============================================================
    def frame_promociones():
        cont = tk.Frame(main_frame, bg=COLOR_FONDO)
        cont.pack(fill="both", expand=True, pady=10)

        tk.Label(cont, text="Gestión de Descuentos y Promociones",
                 font=FONT_SUBTITULO, bg=COLOR_FONDO).pack(pady=10)

        form = tk.Frame(cont, bg=COLOR_FONDO)
        form.pack(pady=5)

        tk.Label(form, text="Nombre del producto:", bg=COLOR_FONDO, font=FONT_TEXTO).grid(row=0, column=0, sticky="e", padx=5, pady=5)
        nombre_entry = tk.Entry(form, font=FONT_TEXTO, width=30)
        nombre_entry.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(form, text="Precio original:", bg=COLOR_FONDO, font=FONT_TEXTO).grid(row=1, column=0, sticky="e", padx=5, pady=5)
        precio_original_entry = tk.Entry(form, font=FONT_TEXTO, width=20)
        precio_original_entry.grid(row=1, column=1, padx=5, pady=5)

        tk.Label(form, text="Precio con descuento:", bg=COLOR_FONDO, font=FONT_TEXTO).grid(row=2, column=0, sticky="e", padx=5, pady=5)
        precio_desc_entry = tk.Entry(form, font=FONT_TEXTO, width=20)
        precio_desc_entry.grid(row=2, column=1, padx=5, pady=5)

        foto_label = tk.Label(form, bg=COLOR_FONDO)
        foto_label.grid(row=0, column=2, rowspan=3, padx=10)

        ruta_foto = {"path": None}

        def seleccionar_foto_promocion():
            ruta = filedialog.askopenfilename(filetypes=[("Imágenes", "*.png;*.jpg")])
            if ruta:
                ruta_foto["path"] = ruta
                try:
                    ...
                    foto_label.config(text="Imagen inválida")

        tk.Button(form, text="Agregar Foto", bg=COLOR_BOTON, fg="white",
                  width=12, height=1, font=FONT_BOTON, command=seleccionar_foto_promocion).grid(row=3, column=2, pady=6)

        def agregar_promocion():
            nombre = nombre_entry.get().strip()
            po = precio_original_entry.get().strip()
            pd = precio_desc_entry.get().strip()

            if not nombre:
                messagebox.showerror("Error", "Ingrese el nombre del producto.")
                return
            try:
                po_f = float(str(po).replace(",", "."))
                pd_f = float(str(pd).replace(",", "."))
            except:
                messagebox.showerror("Error", "Precios inválidos.")
                return
            if pd_f >= po_f:
                messagebox.showwarning("Atención", "El precio con descuento debe ser menor que el precio original.")
                return

            promociones.append({
                "nombre": nombre,
                "precio_original": po_f,
                "precio_descuento": pd_f,
                "foto": ruta_foto["path"]
            })

            # limpiar formulario
            nombre_entry.delete(0, tk.END)
            precio_original_entry.delete(0, tk.END)
            precio_desc_entry.delete(0, tk.END)
            ruta_foto["path"] = None
            foto_label.config(image="", text="")

            actualizar_promociones()

        tk.Button(form, text="Agregar Promoción", bg=COLOR_BOTON, fg="white",
                  width=18, height=2, font=FONT_BOTON, command=agregar_promocion).grid(row=4, column=1, pady=10)

        # LISTADO DE PROMOCIONES
        tk.Label(cont, text="Promociones Actuales", font=FONT_SUBTITULO, bg=COLOR_FONDO).pack(pady=8)
        ...
        listado = tk.Frame(canvas, bg=COLOR_FONDO)
        canvas.create_window((0, 0), window=listado, anchor="n")

        def actualizar_promociones():
            for w in listado.winfo_children():
                w.destroy()

            if not promociones:
                tk.Label(listado, text="No hay promociones.", bg=COLOR_FONDO,
                         font=FONT_TEXTO).pack(anchor="center", pady=10)
                return

            for i, pr in enumerate(promociones):
                card = tk.Frame(listado, bg="#e8f4d9", bd=2, relief="solid")
                card.pack(pady=8, padx=10, fill="x")

                if pr["foto"]:
                    try:
                        img = Image.open(pr["foto"])
                        img = img.resize((90, 90))
                        imgTk = ImageTk.PhotoImage(img)
                        img_lbl = tk.Label(card, image=imgTk, bg="#e8f4d9", cursor="hand2")
                        img_lbl.image = imgTk
                        img_lbl.pack(side="left", padx=10)
                        img_lbl.bind("<button-1>", lambda e, ruta=pr["foto"]: abrir_imagen(ruta))
                    except:
                        pass

                info = tk.Frame(card, bg="#e8f4d9")
                info.pack(side="left", padx=6)

                tk.Label(info, text=f"{pr['nombre']}", font=("Arial", 16, "bold"), bg="#e8f4d9").pack(anchor="w")
                tk.Label(info, text=f"Original: ${pr['precio_original']:.2f}", font=FONT_TEXTO, bg="#e8f4d9").pack(anchor="w")
                tk.Label(info, text=f"Descuento: ${pr['precio_descuento']:.2f}", font=FONT_TEXTO, bg="#e8f4d9").pack(anchor="w")

                btns = tk.Frame(card, bg="#e8f4d9")
                btns.pack(side="right", padx=10)
                tk.Button(btns, text="Eliminar", bg="red", fg="white",
                          command=lambda x=i: eliminar_promocion(x)).pack()

            listado.update_idletasks()
            canvas.configure(scrollregion=canvas.bbox("all"))

        def eliminar_promocion(i):
            promociones.pop(i)
            actualizar_promociones()

        listado.bind("<configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))
        actualizar_promociones()
    ...
    # ----- NUEVO: Gestión de Descuentos y Promociones -----
    promo_index = menu.index("end") + 1
    menu.add_command(label="Gestión de Descuentos", state="disabled",
                     command=lambda: mostrar_frame(frame_promociones))
